...
seekingalphasearch.setResultOrder(google.search.Search.ORDER_BY_RELEVANCE );
       ...
       google.search.Search.getBranding('branding');
		...
		seekingalphasearchRelevence.execute(search);
        google.search.Search.getBranding('branding');
		...
		if(search!=''){
			...
			success: function(response){
				...
				if(f == '-1'){
					...
					var g = comman_title.indexOf( title );
				if(g == '-1'){
					if(i <=10){
				...
				var h = comman_title.indexOf( title );
				if(h == '-1'){
					if(i <=5){
					...
					var j = comman_title.indexOf( title );
				if(j == '-1'){
					if(i <=10){
	...
	if(search!=''){
			...
			//	if(for_search_part=='for_15_1'){
				...
				//	var link=$(this).find(".search-result-title").children('a').attr("href");				
					...
					//var byline=$(this).find(".byline").html();
					//var link=$(this).find(".search-result-title").children('a').attr("href");
					$(site_content1).find('.gsc-expansionArea').find('.gsc-webResult').each(function(){
									
									
					var title=$(this).find('.gs-webResult').find('.gsc-thumbnail-inside').find(".gs-title").children('a').html();
					var link=$(this).find('.gs-webResult').find('.gsc-thumbnail-inside').find(".gs-title").children('a').attr("href");		
					var desc=$(this).find(".gs-snippet").html();
 ...
 Indicate if, in your opinion, the companies identified in the results would be potential collaborators and/or licensees of your invention.
			...
			<label>15.a Input 3-8 key words for <b style="text-decoration:underline">inventive technology</b> and <b style="text-decoration:underline">impact area</b> (akin to journal key words).. Indicate if the companies (within the search results) would be scientific collaborator(s) or licensee(s) of the invention, if patented.
				...
				<div id="content" style="display: none;">Loading...</div>
					<div>
						<div class="prior_inventions_result customb" id="result" >hhhh h
						<?php
