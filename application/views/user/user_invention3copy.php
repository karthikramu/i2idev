    ...
    ?>
	  ...
	  function onLoad3(search) {
		...
		*/
        ...
        seekingalphasearch = new google.search.WebSearch();
        seekingalphasearch.setSearchCompleteCallback(this, searchCompleteMedical, null);
		seekingalphasearch.setResultSetSize(8);
		//seekingalphasearch.execute(search_string+' site:http://seekingalpha.com');
		seekingalphasearch.execute(search+' site:http://seekingalpha.com');
        google.search.Search.getBranding('branding3');
...
$(document).ready(function(){
	...
	$("input[name='search_15']").live('keyup',function(){
		...
		if(search_rele!=''){
			...
			data: { id : id , search_rele : search_rele },
			type: "POST",
			dataType: "Json",
			success: function(response){
			
				var result_rel = eval(response);
			
				for_search_part=result_rel.for_search_part;
				...
				if(for_search_part=='for_15_1'){
					...
					site_content_rel_1=result_rel.site_content_rel_1;
					site_content_rel_2=result_rel.site_content_rel_2;
					site_content_rel_3=result_rel.site_content_rel_3;
					
				
					var i = 1 ; 
					var return_html1="";
					$(site_content_rel_1).find('.search-results').find('.search-result').each(function(){	 
					//if(i <=5){
					//alert($(this).find(".title").children('a').html());
					var title=$(this).find(".search-result-title").children('a').html();
					var link=$(this).find(".search-result-title").children('a').attr("href");
				...
				if(b == '-1'){
					...
					$(site_content_rel_2).find('.search-results').find('.search-result').each(function(){	 
					//if(i <=10){
					//alert($(this).find(".title").children('a').html());
					var title=$(this).find(".search-result-title").children('a').html();
					var link=$(this).find(".search-result-title").children('a').attr("href");
				...
				if(c == '-1'){
					...
					i++;
						...
						$(site_content_rel_3).find('.search-results').find('.search-result').each(function(){	 
					//if(i <=10){
					//alert($(this).find(".title").children('a').html());
					var title=$(this).find(".search-result-title").children('a').html();
					var link=$(this).find(".search-result-title").children('a').attr("href");
				...
				if(for_search_part=='for_15_2'){
					...
					site_content_rel_1=result.site_content_rel_1;
					site_content_rel_2=result.site_content_rel_2;
				...
				if(d == '-1'){
					...
					var e = comman_title_rele.indexOf( title );
				if(e == '-1'){
					if(i <=10){
				...
				if(for_search_part=='for_15_3'){
					...
					var desc=$(this).find(".search-snippet").html();
					//var byline=$(this).find(".byline").html();
					var f = comman_title_rele.indexOf( title );
				if(f == '-1'){
					if(i <=10){
